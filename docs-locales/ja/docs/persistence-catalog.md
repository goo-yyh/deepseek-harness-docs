...
セッションの耐久イベントログに記録できるすべてのイベント型を示します。完全な永続化済み `SessionEvent` エンベロープと、マージ拡張可能な `SessionEventMap` の各メンバーを対象にします。このリポジトリでは、`@deepseek-ai/dsh-session` 内の所有ボキャブラリとすべてのプラグイン宣言が `@deepseek-ai/dsh-session/types` にマージされます。ソース JSDoc、完全なペイロード宣言、サーフェスバッジ、宣言箇所を掲載しています。これは、[session.md](subsystems/session.md)（サーフェスの順序と `deriveMessages()` プロジェクション）、[persistence.md](subsystems/persistence.md)（ログを耐久化する方法）、および [session.md](subsystems/session.md#cordis-surface) の生成領域（ライブバスの配線。ログイベントは cordis イベントではありません。単一の `session/event` emit を介してリスナーに届きます）を補完します。

このファイルはソース（`scripts/gen-persistence-catalog.ts`）から生成され、`pnpm run verify-persistence-catalog`（`doc-sync` の一部）によって最新であることが検証されます。手動で編集しないでください。宣言ブロックは、ソース宣言とネストしたプロパティ JSDoc を保持し、包含するインターフェースまたはモジュールによるインデントのみを取り除きます。また、`ts persistence-catalog` フェンスを使用します（宣言が所有モジュールの型を参照するため、doc-typecheck ではスキップされます）。ペイロード内の型名は、それらを説明するページにリンクします。[永続化ログカタログの Agent Note](../.agents/notes/archived/process/2026-07-04-persistence-log-catalog.md) を参照してください。

以下のエンベロープ宣言は、各イベントの `type`、単調増加の `seq`、エポックミリ秒の `time`、`data`、オプションの `ignorable` 不明型スキップマーカー、および条件付きの `surfaceOp`/`sourceEventSeqs` フィールドで構成されます。**surface** は `SurfaceEventType` メンバーを示します。これは LLM メッセージを生成し、サーフェスリストへの結合方法を宣言します。**log-only** はその他すべてを示します。派生履歴には寄与しない、耐久的で再生可能なレコードです。すべてのペイロードは JSON シリアライズ可能です（`Session.append` で強制されます）。また、形式全体は `SESSION_FORMAT_VERSION = 0` に固定されています。プレリリースのため、互換性は保証されません（[バージョンに関する方針](subsystems/persistence.md)）。対象範囲はこのリポジトリ内のパッケージです。ダウンストリームプラグインは追加のイベント型をマージできますが、それらは設計上このカタログの対象外です。
...
ソース: [`packages/core/session/src/types.ts:336`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:343`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:372`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:404`](../packages/core/session/src/types.ts)
...
ソース: [`packages/core/agent/src/types.ts:19`](../packages/core/agent/src/types.ts)
...
ソース: [`packages/preset/agent-presets/src/session.ts:26`](../packages/preset/agent-presets/src/session.ts)
...
型: [CallId](subsystems/core.md)

ソース: [`packages/interaction/user-approval/src/index.ts:44`](../packages/interaction/user-approval/src/index.ts)

<a id="approvaldecided--log-only"></a>

#### `approval/decided` — ログのみ
...
ソース: [`packages/interaction/user-approval/src/index.ts:55`](../packages/interaction/user-approval/src/index.ts)

<a id="approvalpolicy--log-only"></a>

#### `approval/policy` — ログのみ
...
ソース: [`packages/interaction/user-approval/src/index.ts:67`](../packages/interaction/user-approval/src/index.ts)

### `assistant/*`

<a id="assistantchunk--log-only"></a>

#### `assistant/chunk` — ログのみ
...
型: [StreamChunk](subsystems/llm-streaming.md)

ソース: [`packages/core/session/src/types.ts:266`](../packages/core/session/src/types.ts)

<a id="assistantmessage--surface"></a>

#### `assistant/message` — サーフェス
...
型: [TokenUsage](subsystems/llm-streaming.md)

ソース: [`packages/core/session/src/types.ts:273`](../packages/core/session/src/types.ts)
...
ソース: [`packages/interaction/commands/src/types.ts:95`](../packages/interaction/commands/src/types.ts)
...
ソース: [`packages/interaction/commands/src/types.ts:88`](../packages/interaction/commands/src/types.ts)
...
ソース: [`packages/compaction/compaction/src/types.ts:71`](../packages/compaction/compaction/src/types.ts)

<a id="compactionprune--log-only"></a>

#### `compaction/prune` — ログのみ
...
ソース: [`packages/compaction/compaction/src/types.ts:81`](../packages/compaction/compaction/src/types.ts)

<a id="compactionstart--log-only"></a>

#### `compaction/start` — ログのみ
...
ソース: [`packages/compaction/compaction/src/types.ts:23`](../packages/compaction/compaction/src/types.ts)

<a id="compactionsummary--log-only"></a>

#### `compaction/summary` — ログのみ
...
型: [ContentBlock](subsystems/core.md) · [TokenUsage](subsystems/llm-streaming.md)

ソース: [`packages/compaction/compaction/src/types.ts:33`](../packages/compaction/compaction/src/types.ts)

### `feedback/*`

<a id="feedbackrecord--log-only"></a>

#### `feedback/record` — ログのみ
...
ソース: [`packages/feedback/command-feedback/src/index.ts:62`](../packages/feedback/command-feedback/src/index.ts)
...
ソース: [`packages/goal/goal/src/domain.ts:66`](../packages/goal/goal/src/domain.ts)
...
ソース: [`packages/hooks/hook-protocol/src/types.ts:19`](../packages/hooks/hook-protocol/src/types.ts)
...
ソース: [`packages/hooks/hook-protocol/src/types.ts:31`](../packages/hooks/hook-protocol/src/types.ts)
...
ソース: [`packages/llm/llm-retry/src/types.ts:9`](../packages/llm/llm-retry/src/types.ts)

<a id="llmretry-started--log-only"></a>

#### `llm/retry-started` — ログのみ
...
ソース: [`packages/llm/llm-retry/src/types.ts:11`](../packages/llm/llm-retry/src/types.ts)

### `permission/*`

<a id="permissionpreset--log-only"></a>

#### `permission/preset` — ログのみ
...
ソース: [`packages/interaction/permission-presets/src/index.ts:50`](../packages/interaction/permission-presets/src/index.ts)
...
ソース: [`packages/plan/plan-mode/src/index.ts:53`](../packages/plan/plan-mode/src/index.ts)

### `request/*`

<a id="requestcontext--log-only"></a>

#### `request/context` — ログのみ
...
ソース: [`packages/core/session/src/types.ts:309`](../packages/core/session/src/types.ts)
...
ソース: [`packages/core/session/src/types.ts:304`](../packages/core/session/src/types.ts)
...
ソース: [`packages/sandbox/sandbox-policy/src/session-mode.ts:33`](../packages/sandbox/sandbox-policy/src/session-mode.ts)

### `schedule/*`

<a id="schedulechange--log-only"></a>

#### `schedule/change` — ログのみ
...
型: [ScheduleChange](subsystems/schedule.md)

ソース: [`packages/schedule/schedule/src/types.ts:219`](../packages/schedule/schedule/src/types.ts)

### `session/*`

<a id="sessionend-seed--log-only"></a>

#### `session/end-seed` — ログのみ
...
ソース: [`packages/core/session/src/types.ts:332`](../packages/core/session/src/types.ts)
...
型: [SessionTitleEventData](subsystems/session-title.md)

ソース: [`packages/session/session-title/src/index.ts:100`](../packages/session/session-title/src/index.ts)

<a id="sessiontitle-llm-request--log-only"></a>

#### `session/title-llm-request` — ログのみ
...
型: [SessionTitleLlmRequestEventData](subsystems/session-title.md)

ソース: [`packages/session/session-title-llm/src/index.ts:43`](../packages/session/session-title-llm/src/index.ts)
...
ソース: [`packages/core/session/src/types.ts:256`](../packages/core/session/src/types.ts)
...
ソース: [`packages/core/session/src/types.ts:254`](../packages/core/session/src/types.ts)

### `subagent/*`

<a id="subagentdescriptor--log-only"></a>

#### `subagent/descriptor` — ログのみ
...
ソース: [`packages/subagent/subagent/src/descriptor.ts:37`](../packages/subagent/subagent/src/descriptor.ts)
...
型: [TodoItem](subsystems/session.md)

ソース: [`packages/core/session/src/types.ts:299`](../packages/core/session/src/types.ts)
...
ソース: [`packages/core/session/src/types.ts:279`](../packages/core/session/src/types.ts)

<a id="toolcode-dispatch--log-only"></a>

#### `tool/code-dispatch` — ログのみ
...
ソース: [`packages/core/tools/src/types.ts:56`](../packages/core/tools/src/types.ts)

<a id="toolcode-dispatch-start--log-only"></a>

#### `tool/code-dispatch-start` — ログのみ
...
ソース: [`packages/core/tools/src/types.ts:40`](../packages/core/tools/src/types.ts)
...
ソース: [`packages/core/session/src/types.ts:291`](../packages/core/session/src/types.ts)

### `tool-workflow/*`

<a id="tool-workflowagent-end--log-only"></a>

#### `tool-workflow/agent-end` — ログのみ
...
ソース: [`packages/workflow/tool-workflow/src/types.ts:57`](../packages/workflow/tool-workflow/src/types.ts)

<a id="tool-workflowagent-start--log-only"></a>

#### `tool-workflow/agent-start` — ログのみ
...
ソース: [`packages/workflow/tool-workflow/src/types.ts:52`](../packages/workflow/tool-workflow/src/types.ts)

<a id="tool-workflowrun-end--log-only"></a>

#### `tool-workflow/run-end` — ログのみ
...
ソース: [`packages/workflow/tool-workflow/src/types.ts:62`](../packages/workflow/tool-workflow/src/types.ts)

<a id="tool-workflowrun-start--log-only"></a>

#### `tool-workflow/run-start` — ログのみ
...
ソース: [`packages/workflow/tool-workflow/src/types.ts:47`](../packages/workflow/tool-workflow/src/types.ts)
...
型: [TurnEndReason](subsystems/session.md)

ソース: [`packages/core/session/src/types.ts:252`](../packages/core/session/src/types.ts)
...
ソース: [`packages/core/session/src/types.ts:243`](../packages/core/session/src/types.ts)

### `user/*`

<a id="usermessage--surface"></a>

#### `user/message` — 公開インターフェース
...
ソース: [`packages/core/session/src/types.ts:264`](../packages/core/session/src/types.ts)

### `web/*`

<a id="webdeepseek-search-llm-request--log-only"></a>

#### `web/deepseek-search-llm-request` — ログ専用
...
ソース: [`packages/web/web-search-deepseek/src/provider.ts:83`](../packages/web/web-search-deepseek/src/provider.ts)
