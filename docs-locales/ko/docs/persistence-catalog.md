...
세션의 내구성 이벤트 로그에 나타날 수 있는 모든 이벤트 유형을 다룹니다. 즉, 완전하게 영속화된 `SessionEvent` 봉투와 병합 확장이 가능한 `SessionEventMap`의 각 멤버입니다. 이 리포지토리에서는 `@deepseek-ai/dsh-session`의 소유 어휘와 모든 플러그인 선언이 `@deepseek-ai/dsh-session/types`로 병합됩니다. 여기에는 소스 JSDoc, 전체 페이로드 선언, 표면 배지, 선언 위치가 포함됩니다. 이는 [session.md](subsystems/session.md)(표면 순서 및 `deriveMessages()` 프로젝션), [persistence.md](subsystems/persistence.md)(로그를 내구적으로 만드는 방법), 그리고 [session.md](subsystems/session.md#cordis-surface)의 생성된 영역(라이브 버스 연결 — 로그 이벤트는 cordis 이벤트가 아니며, 단일 `session/event` emit을 통해 리스너에 도달함)을 보완합니다.

이 파일은 소스(`scripts/gen-persistence-catalog.ts`)에서 GENERATED되며 `pnpm run verify-persistence-catalog`(`doc-sync`의 일부)가 최신 상태인지 검증합니다. 직접 편집하지 마세요. 선언 블록은 포함된 인터페이스/모듈이 부여한 들여쓰기만 제거하고 소스 선언 및 중첩된 속성 JSDoc을 유지하며, `ts persistence-catalog` 펜스를 사용합니다(선언이 소유 모듈의 타입을 참조하므로 doc-typecheck에서 건너뜀). 페이로드의 타입 이름은 해당 타입을 문서화하는 페이지에 연결됩니다. [영속성 로그 카탈로그 Agent Note](../.agents/notes/archived/process/2026-07-04-persistence-log-catalog.md)를 참조하세요.

아래의 봉투 선언은 각 이벤트의 `type`, 단조 증가하는 `seq`, epoch-ms `time`, `data`, 선택적 `ignorable` 알 수 없는 유형 건너뛰기 마커 및 조건부 `surfaceOp`/`sourceEventSeqs` 필드로 구성됩니다. **surface** 는 `SurfaceEventType` 멤버를 표시합니다. 이는 LLM 메시지를 생성하고 표면 목록에 포함되는 방식을 선언합니다. **log-only** 는 그 외 모든 항목을 표시합니다. 즉, 파생 이력에 기여하지 않는 내구성 있고 재생 가능한 레코드입니다. 모든 페이로드는 JSON 직렬화가 가능하며(`Session.append`에서 강제됨), 전체 형식은 `SESSION_FORMAT_VERSION = 0`에 고정됩니다. 릴리스 전 단계이므로 호환성이 보장되지 않습니다([버전 방침](subsystems/persistence.md)). 범위: 이 리포지토리의 패키지입니다. 다운스트림 플러그인은 추가 이벤트 유형을 병합할 수 있으며, 이는 구조상 이 카탈로그의 범위 밖입니다.
...
소스: [`packages/core/session/src/types.ts:336`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:343`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:372`](../packages/core/session/src/types.ts) · [`packages/core/session/src/types.ts:404`](../packages/core/session/src/types.ts)
...
소스: [`packages/core/agent/src/types.ts:19`](../packages/core/agent/src/types.ts)
...
소스: [`packages/preset/agent-presets/src/session.ts:26`](../packages/preset/agent-presets/src/session.ts)
...
타입: [CallId](subsystems/core.md)

소스: [`packages/interaction/user-approval/src/index.ts:44`](../packages/interaction/user-approval/src/index.ts)
...
소스: [`packages/interaction/user-approval/src/index.ts:55`](../packages/interaction/user-approval/src/index.ts)
...
소스: [`packages/interaction/user-approval/src/index.ts:67`](../packages/interaction/user-approval/src/index.ts)
...
유형: [StreamChunk](subsystems/llm-streaming.md)

소스: [`packages/core/session/src/types.ts:266`](../packages/core/session/src/types.ts)
...
유형: [TokenUsage](subsystems/llm-streaming.md)

소스: [`packages/core/session/src/types.ts:273`](../packages/core/session/src/types.ts)
...
소스: [`packages/interaction/commands/src/types.ts:95`](../packages/interaction/commands/src/types.ts)
...
소스: [`packages/interaction/commands/src/types.ts:88`](../packages/interaction/commands/src/types.ts)
...
소스: [`packages/compaction/compaction/src/types.ts:71`](../packages/compaction/compaction/src/types.ts)
...
소스: [`packages/compaction/compaction/src/types.ts:81`](../packages/compaction/compaction/src/types.ts)
...
소스: [`packages/compaction/compaction/src/types.ts:23`](../packages/compaction/compaction/src/types.ts)
...
유형: [ContentBlock](subsystems/core.md) · [TokenUsage](subsystems/llm-streaming.md)

소스: [`packages/compaction/compaction/src/types.ts:33`](../packages/compaction/compaction/src/types.ts)
...
소스: [`packages/feedback/command-feedback/src/index.ts:62`](../packages/feedback/command-feedback/src/index.ts)
...
소스: [`packages/goal/goal/src/domain.ts:66`](../packages/goal/goal/src/domain.ts)
...
소스: [`packages/hooks/hook-protocol/src/types.ts:19`](../packages/hooks/hook-protocol/src/types.ts)
...
소스: [`packages/hooks/hook-protocol/src/types.ts:31`](../packages/hooks/hook-protocol/src/types.ts)
...
소스: [`packages/llm/llm-retry/src/types.ts:9`](../packages/llm/llm-retry/src/types.ts)
...
소스: [`packages/llm/llm-retry/src/types.ts:11`](../packages/llm/llm-retry/src/types.ts)
...
소스: [`packages/interaction/permission-presets/src/index.ts:50`](../packages/interaction/permission-presets/src/index.ts)
...
소스: [`packages/plan/plan-mode/src/index.ts:53`](../packages/plan/plan-mode/src/index.ts)
...
소스: [`packages/core/session/src/types.ts:309`](../packages/core/session/src/types.ts)
...
소스: [`packages/core/session/src/types.ts:304`](../packages/core/session/src/types.ts)
...
소스: [`packages/sandbox/sandbox-policy/src/session-mode.ts:33`](../packages/sandbox/sandbox-policy/src/session-mode.ts)
...
유형: [ScheduleChange](subsystems/schedule.md)

소스: [`packages/schedule/schedule/src/types.ts:219`](../packages/schedule/schedule/src/types.ts)
...
소스: [`packages/core/session/src/types.ts:332`](../packages/core/session/src/types.ts)
...
유형: [SessionTitleEventData](subsystems/session-title.md)

소스: [`packages/session/session-title/src/index.ts:100`](../packages/session/session-title/src/index.ts)
...
유형: [SessionTitleLlmRequestEventData](subsystems/session-title.md)

소스: [`packages/session/session-title-llm/src/index.ts:43`](../packages/session/session-title-llm/src/index.ts)
...
소스: [`packages/core/session/src/types.ts:256`](../packages/core/session/src/types.ts)
...
소스: [`packages/core/session/src/types.ts:254`](../packages/core/session/src/types.ts)
...
소스: [`packages/subagent/subagent/src/descriptor.ts:37`](../packages/subagent/subagent/src/descriptor.ts)
...
유형: [TodoItem](subsystems/session.md)

소스: [`packages/core/session/src/types.ts:299`](../packages/core/session/src/types.ts)
...
유형: [CallId](subsystems/core.md)

소스: [`packages/core/session/src/types.ts:279`](../packages/core/session/src/types.ts)
...
소스: [`packages/core/tools/src/types.ts:56`](../packages/core/tools/src/types.ts)
...
소스: [`packages/core/tools/src/types.ts:40`](../packages/core/tools/src/types.ts)
...
소스: [`packages/core/session/src/types.ts:291`](../packages/core/session/src/types.ts)
...
소스: [`packages/workflow/tool-workflow/src/types.ts:57`](../packages/workflow/tool-workflow/src/types.ts)
...
소스: [`packages/workflow/tool-workflow/src/types.ts:52`](../packages/workflow/tool-workflow/src/types.ts)
...
소스: [`packages/workflow/tool-workflow/src/types.ts:62`](../packages/workflow/tool-workflow/src/types.ts)
...
소스: [`packages/workflow/tool-workflow/src/types.ts:47`](../packages/workflow/tool-workflow/src/types.ts)
...
유형: [TurnEndReason](subsystems/session.md)

소스: [`packages/core/session/src/types.ts:252`](../packages/core/session/src/types.ts)
...
출처: [`packages/core/session/src/types.ts:243`](../packages/core/session/src/types.ts)
...
출처: [`packages/core/session/src/types.ts:264`](../packages/core/session/src/types.ts)
...
출처: [`packages/web/web-search-deepseek/src/provider.ts:83`](../packages/web/web-search-deepseek/src/provider.ts)
