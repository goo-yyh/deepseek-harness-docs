...
각 `config:` 블록은 로드 가능한 각 harness 패키지에 대해 `cordis.yml` 항목에서 설정할 수 있는 내용을 지정합니다. 즉, 해당 패키지의 `apply` 함수 또는 서비스 생성자가 받는 원문 그대로의 구성 선언(JSDoc 포함)과 함께, 참조하는 모든 타입을 병기(패키지 로컬 타입)하거나 링크(그 외 모든 타입)합니다. 붙여 넣은 내용은 플러그인이 선언한 전체 구성 타입입니다. 런타임 스키마가 의도적으로 제외하는 필드는 런타임 전용 이음새이며(자체 JSDoc에 그렇게 명시됨) `cordis.yml`에서 설정할 수 없습니다. 이는 **배포** 축 참조입니다. 플러그인 작성자가 사용하는 연결 방식은 각 [하위 시스템 페이지](subsystems/core.md)의 생성된 Cordis API 영역에 있으며, 모델 대상 도구 스키마는 [도구 카탈로그](tool-catalog.md)에 있고, [subsystems/](subsystems/core.md)에는 이러한 선언이 참조하는 타입이 문서화되어 있습니다.

이 파일은 소스(`scripts/gen-config-catalog.ts`)에서 생성되며 `pnpm run verify-config-catalog`(`doc-sync`의 일부)로 최신 상태인지 검증됩니다. 직접 수정하지 마세요. 선언 블록은 `ts config-catalog` 펜스를 사용합니다. 가져오기를 참조하는 단독 선언은 독립적으로 컴파일할 수 없으므로 문서 타입 검사에서 건너뜁니다. 생성기는 런타임 schemastery 스키마와 붙여 넣은 선언도 교차 검사합니다. 중첩 키를 포함한 모든 스키마 검증 키는 선언된 구성 타입에서 찾을 수 있어야 하므로, 붙여 넣기 내용으로 로더가 허용하는 필드를 숨길 수 없습니다.

`Requires:` 줄에는 플러그인이 `inject`하는 서비스 키가 나열됩니다. 해당 `cordis.yml` 트리도 이 서비스의 제공자를 로드해야 합니다. 범위는 harness 계층(`packages/`)입니다. 구성 트리가 추가로 로드할 수 있는 vendored cordis 플러그인(`hmr`, 콘솔 로거 등)은 고정된 업스트림 소스([벤더링 정책](../vendor/README.md))이며 여기에는 카탈로그화되지 않습니다.
...
의존 대상: `Stream` (`@agentclientprotocol/sdk`)

소스: [`packages/acp/acp/src/index.ts:70`](../packages/acp/acp/src/index.ts)
...
의존 대상: [`agentCore`](../packages/examples/agent-spine-demo/src/index.ts) · [`JsonlCompression`](../packages/session/session-persistence-jsonl/src/index.ts) · [`ToolsConfig`](#deepseek-aidsh-tools)

소스: [`packages/examples/acp-demo/src/index.ts:39`](../packages/examples/acp-demo/src/index.ts)
...
소스: [`packages/core/agent-default-model/src/index.ts:41`](../packages/core/agent-default-model/src/index.ts)
...
소스: [`packages/context/agent-instructions/src/config.ts:18`](../packages/context/agent-instructions/src/config.ts)

<a id="deepseek-aidsh-agent-loop"></a>

## `@deepseek-ai/dsh-agent-loop`

필수 항목: `agents` · `sessions` · `llm` · `tools` · `systemPrompt`
...
종속 항목: [`AgentOptions`](subsystems/core.md) · [`SessionId`](subsystems/core.md)

소스: [`packages/core/agent-loop/src/index.ts:255`](../packages/core/agent-loop/src/index.ts)
...
소스: [`packages/preset/agent-presets/src/preset.ts:52`](../packages/preset/agent-presets/src/preset.ts)
...
종속 항목: [`ToolPresentationMode`](subsystems/tools.md)

소스: [`packages/core/agent-tool-presentation/src/index.ts:38`](../packages/core/agent-tool-presentation/src/index.ts)
...
소스: [`packages/attachment/attachment-local/src/index.ts:24`](../packages/attachment/attachment-local/src/index.ts)
...
소스: [`packages/shell/bash-local/src/index.ts:41`](../packages/shell/bash-local/src/index.ts)

<a id="deepseek-aidsh-bash-sandbox"></a>

## `@deepseek-ai/dsh-bash-sandbox`

필수 항목: `subprocess` · `sandbox` · `sandboxPolicy`
...
종속 항목: [`LocalConfig`](#deepseek-aidsh-bash-local)

소스: [`packages/shell/bash-sandbox/src/index.ts:35`](../packages/shell/bash-sandbox/src/index.ts)
...
소스: [`packages/client/connection/src/index.ts:50`](../packages/client/connection/src/index.ts)

<a id="deepseek-aidsh-client-hmr"></a>

## `@deepseek-ai/dsh-client-hmr`

필수 항목: `clientModules` · `webServer`
...
소스: [`packages/client/hmr/src/index.ts:31`](../packages/client/hmr/src/index.ts)
...
소스: [`packages/code-runtime/code-runtime-worker-thread/src/index.ts:25`](../packages/code-runtime/code-runtime-worker-thread/src/index.ts)

<a id="deepseek-aidsh-compaction-basic"></a>

## `@deepseek-ai/dsh-compaction-basic`

필수 항목: `llm` · `tokenMeter` · `sessions`
...
소스: [`packages/compaction/compaction-basic/src/types.ts:38`](../packages/compaction/compaction-basic/src/types.ts)
...
소스: [`packages/compaction/compaction-tool-result-pruner/src/types.ts:4`](../packages/compaction/compaction-tool-result-pruner/src/types.ts)
...
소스: [`packages/extensions/cordis-host-runner/src/index.ts:88`](../packages/extensions/cordis-host-runner/src/index.ts)
...
소스: [`packages/credentials/credentials-local/src/index.ts:55`](../packages/credentials/credentials-local/src/index.ts)
...
소스: [`packages/e2b/e2b/src/index.ts:43`](../packages/e2b/e2b/src/index.ts)
...
소스: [`packages/fs/fs-local/src/index.ts:41`](../packages/fs/fs-local/src/index.ts)
...
의존 항목: [`LocalConfig`](#deepseek-aidsh-fs-local)

소스: [`packages/fs/fs-sandbox/src/index.ts:49`](../packages/fs/fs-sandbox/src/index.ts)
...
소스: [`packages/goal/goal/src/index.ts:116`](../packages/goal/goal/src/index.ts)

<a id="deepseek-aidsh-headless"></a>

## `@deepseek-ai/dsh-headless`

필수 항목: `agentDefaultModel` · `agents` · `sessions`
...
소스: [`packages/bundle/headless/src/index.ts:31`](../packages/bundle/headless/src/index.ts)
...
소스: [`packages/hooks/hooks-claude-code/src/index.ts:45`](../packages/hooks/hooks-claude-code/src/index.ts)
...
소스: [`packages/hooks/hooks-codex/src/index.ts:44`](../packages/hooks/hooks-codex/src/index.ts)

<a id="deepseek-aidsh-host-apiproxy"></a>

## `@deepseek-ai/dsh-host-apiproxy`

필수 항목: `agentDefaultModel` · `agents` · `attachments` · `directoryPicker` · `llm` · `sessions` · `subagents` · `sessionQuery` · `tools` · `userQuestions` · `workspaceRegistry`
...
출처: [`packages/host/apiproxy/src/index.ts:41`](../packages/host/apiproxy/src/index.ts)
...
출처: [`packages/host/directory-picker-browse/src/index.ts:181`](../packages/host/directory-picker-browse/src/index.ts)
...
출처: [`packages/host/frontend-static/src/index.ts:28`](../packages/host/frontend-static/src/index.ts)
...
출처: [`packages/host/webserver/src/index.ts:45`](../packages/host/webserver/src/index.ts)
...
출처: [`packages/runtime-diagnostics/invariants/src/index.ts:15`](../packages/runtime-diagnostics/invariants/src/index.ts)
...
출처: [`packages/jobs/jobs-local/src/index.ts:31`](../packages/jobs/jobs-local/src/index.ts)
...
의존 대상: [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts)

출처: [`packages/llm/llm-deepseek/src/index.ts:62`](../packages/llm/llm-deepseek/src/index.ts)
...
의존 항목: `Api` (`@earendil-works/pi-ai`) · `CacheRetention` (`@earendil-works/pi-ai`) · `Model` (`@earendil-works/pi-ai`) · `ModelThinkingLevel` (`@earendil-works/pi-ai`) · `OpenAICompletionsCompat` (`@earendil-works/pi-ai`) · [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts) · `ThinkingBudgets` (`@earendil-works/pi-ai`) · `Transport` (`@earendil-works/pi-ai`)

소스: [`packages/llm/llm-pi-ai/src/config.ts:172`](../packages/llm/llm-pi-ai/src/config.ts)
...
의존 항목: [`ModelModality`](../packages/llm/llm/src/index.ts) · [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts)

소스: [`packages/test-support/llm-replay/src/index.ts:776`](../packages/test-support/llm-replay/src/index.ts)
...
소스: [`packages/llm/llm-retry/src/index.ts:24`](../packages/llm/llm-retry/src/index.ts)

<a id="deepseek-aidsh-lsp-stdio"></a>

## `@deepseek-ai/dsh-lsp-stdio`

필요: `fs` · `lsp` · `subprocess`
...
소스: [`packages/lsp/lsp-stdio/src/index.ts:82`](../packages/lsp/lsp-stdio/src/index.ts)
...
출처: [`packages/mcp/mcp-client/src/index.ts:98`](../packages/mcp/mcp-client/src/index.ts)

<a id="deepseek-aidsh-message-feedback"></a>

## `@deepseek-ai/dsh-message-feedback`

필수 항목: `storageDomain` · `sessionPersistence` · `sessions`
...
출처: [`packages/feedback/message-feedback/src/index.ts:49`](../packages/feedback/message-feedback/src/index.ts)

<a id="deepseek-aidsh-permission-presets"></a>

## `@deepseek-ai/dsh-permission-presets`

필수 항목: `shell` · `approval` · `sessions`
...
의존 항목: [`ApprovalPolicy`](subsystems/approval.md) · [`SandboxMode`](subsystems/sandbox.md)

출처: [`packages/interaction/permission-presets/src/index.ts:140`](../packages/interaction/permission-presets/src/index.ts)
...
출처: [`packages/preset/persona/src/index.ts:34`](../packages/preset/persona/src/index.ts)

<a id="deepseek-aidsh-plan-mode"></a>

## `@deepseek-ai/dsh-plan-mode`

필수 항목: `tools` · `systemPrompt`
...
출처: [`packages/plan/plan-mode/src/index.ts:70`](../packages/plan/plan-mode/src/index.ts)
...
출처: [`packages/shell/pwsh-local/src/index.ts:58`](../packages/shell/pwsh-local/src/index.ts)
...
의존 항목: [`LocalConfig`](#deepseek-aidsh-pwsh-local)

출처: [`packages/shell/pwsh-sandbox/src/index.ts:40`](../packages/shell/pwsh-sandbox/src/index.ts)
...
출처: [`packages/guard/repeat-tool-reminder/src/index.ts:28`](../packages/guard/repeat-tool-reminder/src/index.ts)
...
출처: [`packages/sandbox/sandbox-local/src/index.ts:44`](../packages/sandbox/sandbox-local/src/index.ts)
...
의존 항목: [`SandboxMode`](subsystems/sandbox.md)

출처: [`packages/sandbox/sandbox-policy/src/index.ts:67`](../packages/sandbox/sandbox-policy/src/index.ts)
...
의존 항목: `Readable` (`node:stream`) · `Writable` (`node:stream`)

출처: [`packages/sdk/server/src/index.ts:25`](../packages/sdk/server/src/index.ts)
...
출처: [`packages/session/session-persistence-jsonl/src/index.ts:60`](../packages/session/session-persistence-jsonl/src/index.ts)
...
소스: [`packages/session/session-persistence-sqlite/src/index.ts:70`](../packages/session/session-persistence-sqlite/src/index.ts)

<a id="deepseek-aidsh-session-projection-cache"></a>

## `@deepseek-ai/dsh-session-projection-cache`

필요 사항: `storageDomain` · `sessionProjections` · `sessionPersistence` · `sessions`
...
소스: [`packages/session/session-projection-cache/src/index.ts:42`](../packages/session/session-projection-cache/src/index.ts)
...
종속 대상: [`SessionQueryConfig`](../packages/session-query/session-query/src/index.ts)

소스: [`packages/session-query/session-query-sqlite/src/index.ts:89`](../packages/session-query/session-query-sqlite/src/index.ts)
...
소스: [`packages/context/session-reference/src/config.ts:11`](../packages/context/session-reference/src/config.ts)
...
종속 대상: `BatchLogRecordProcessorOptions` (`@opentelemetry/sdk-logs`) · `OTLPExporterNodeConfigBase` (`@opentelemetry/otlp-exporter-base`)

소스: [`packages/session/session-telemetry-otel/src/index.ts:91`](../packages/session/session-telemetry-otel/src/index.ts)
...
소스: [`packages/session/session-title/src/index.ts:79`](../packages/session/session-title/src/index.ts)

<a id="deepseek-aidsh-session-title-all-prompts-llm"></a>

## `@deepseek-ai/dsh-session-title-all-prompts-llm`

필요 사항: `sessionTitle` · `llm` · `sessions`
...
종속 대상: [`SessionTitleLlmConfig`](../packages/session/session-title-llm/src/index.ts)

소스: [`packages/session/session-title-all-prompts-llm/src/index.ts:15`](../packages/session/session-title-all-prompts-llm/src/index.ts)
...
소스: [`packages/session/session-title-first-prompt-llm/src/index.ts:15`](../packages/session/session-title-first-prompt-llm/src/index.ts)
...
소스: [`packages/settings/settings-file/src/index.ts:21`](../packages/settings/settings-file/src/index.ts)
...
소스: [`packages/shell/shell-env/src/index.ts:29`](../packages/shell/shell-env/src/index.ts)
...
소스: [`packages/skill/skill/src/index.ts:279`](../packages/skill/skill/src/index.ts)
...
소스: [`packages/skill/skill-filesystem/src/index.ts:49`](../packages/skill/skill-filesystem/src/index.ts)
...
소스: [`packages/spill/spill-local/src/index.ts:22`](../packages/spill/spill-local/src/index.ts)
...
소스: [`packages/spill/spill-policy/src/index.ts:60`](../packages/spill/spill-policy/src/index.ts)
...
소스: [`packages/storage/storage-domain/src/index.ts:52`](../packages/storage/storage-domain/src/index.ts)
...
소스: [`packages/storage/storage-json/src/index.ts:27`](../packages/storage/storage-json/src/index.ts)
...
소스: [`packages/storage/storage-sqlite/src/index.ts:24`](../packages/storage/storage-sqlite/src/index.ts)

<a id="deepseek-aidsh-subagent-acp"></a>

## `@deepseek-ai/dsh-subagent-acp`

필요: `subagents` · `subprocess`
...
출처: [`packages/subagent/subagent-acp/src/index.ts:27`](../packages/subagent/subagent-acp/src/index.ts)

<a id="deepseek-aidsh-subagent-claude-code"></a>

## `@deepseek-ai/dsh-subagent-claude-code`

필요 조건: `subagents` · `subprocess`
...
출처: [`packages/subagent/subagent-claude-code/src/index.ts:32`](../packages/subagent/subagent-claude-code/src/index.ts)
...
출처: [`packages/subagent/subagent-codex/src/index.ts:30`](../packages/subagent/subagent-codex/src/index.ts)
...
출처: [`packages/subagent/subagent-dsh-sdk/src/index.ts:29`](../packages/subagent/subagent-dsh-sdk/src/index.ts)
...
출처: [`packages/subagent/subagent-fork-in-process/src/index.ts:31`](../packages/subagent/subagent-fork-in-process/src/index.ts)
...
출처: [`packages/subagent/subagent-spawn-in-process/src/index.ts:25`](../packages/subagent/subagent-spawn-in-process/src/index.ts)
...
출처: [`packages/e2b/subprocess-e2b/src/index.ts:25`](../packages/e2b/subprocess-e2b/src/index.ts)
...
출처: [`packages/core/system-prompt/src/index.ts:186`](../packages/core/system-prompt/src/index.ts)

<a id="deepseek-aidsh-terminal-bash"></a>

## `@deepseek-ai/dsh-terminal-bash`

필요 조건: `terminals` · `sandboxPolicy` · `subprocess`
...
소스: [`packages/terminal/terminal-bash/src/config.ts:6`](../packages/terminal/terminal-bash/src/config.ts)
...
소스: [`packages/context/time-context/src/index.ts:27`](../packages/context/time-context/src/index.ts)
...
소스: [`packages/context/tmux-context/src/index.ts:34`](../packages/context/tmux-context/src/index.ts)
...
소스: [`packages/llm/token-meter/src/types.ts:12`](../packages/llm/token-meter/src/types.ts)

<a id="deepseek-aidsh-tool-bash"></a>

## `@deepseek-ai/dsh-tool-bash`

필수 항목: `tools` · `shell` · `systemPrompt` · `shellEnv`
...
소스: [`packages/shell/tool-bash/src/index.ts:34`](../packages/shell/tool-bash/src/index.ts)

<a id="deepseek-aidsh-tool-bash-persistent"></a>

## `@deepseek-ai/dsh-tool-bash-persistent`

필수 항목: `tools` · `terminals`
...
소스: [`packages/shell/tool-bash-persistent/src/index.ts:405`](../packages/shell/tool-bash-persistent/src/index.ts)

<a id="deepseek-aidsh-tool-fs"></a>

## `@deepseek-ai/dsh-tool-fs`

필수 항목: `tools` · `fs` · `systemPrompt`
...
소스: [`packages/fs/tool-fs/src/index.ts:25`](../packages/fs/tool-fs/src/index.ts)

<a id="deepseek-aidsh-tool-fs-search"></a>

## `@deepseek-ai/dsh-tool-fs-search`

필수 항목: `tools` · `systemPrompt` · `subprocess`
...
소스: [`packages/fs/tool-fs-search/src/index.ts:73`](../packages/fs/tool-fs-search/src/index.ts)

<a id="deepseek-aidsh-tool-goal"></a>

## `@deepseek-ai/dsh-tool-goal`

필수 항목: `agents` · `goals` · `tools` · `systemPrompt`
...
소스: [`packages/goal/tool-goal/src/index.ts:26`](../packages/goal/tool-goal/src/index.ts)

<a id="deepseek-aidsh-tool-jobs"></a>

## `@deepseek-ai/dsh-tool-jobs`

필수 항목: `tools` · `jobs` · `systemPrompt`
...
출처: [`packages/jobs/tool-jobs/src/index.ts:32`](../packages/jobs/tool-jobs/src/index.ts)

<a id="deepseek-aidsh-tool-lsp"></a>

## `@deepseek-ai/dsh-tool-lsp`

필수: `tools` · `lsp` · `systemPrompt`
...
출처: [`packages/lsp/tool-lsp/src/index.ts:58`](../packages/lsp/tool-lsp/src/index.ts)

<a id="deepseek-aidsh-tool-pwsh"></a>

## `@deepseek-ai/dsh-tool-pwsh`

필수: `tools` · `shell` · `systemPrompt` · `shellEnv`
...
출처: [`packages/shell/tool-pwsh/src/index.ts:52`](../packages/shell/tool-pwsh/src/index.ts)

<a id="deepseek-aidsh-tool-ralph"></a>

## `@deepseek-ai/dsh-tool-ralph`

필수: `tools` · `workflowEngine` · `subagents` · `systemPrompt`
...
출처: [`packages/workflow/tool-ralph/src/index.ts:23`](../packages/workflow/tool-ralph/src/index.ts)

<a id="deepseek-aidsh-tool-session-query"></a>

## `@deepseek-ai/dsh-tool-session-query`

필수: `tools` · `systemPrompt` · `sessionQuery`
...
출처: [`packages/session-query/tool-session-query/src/index.ts:29`](../packages/session-query/tool-session-query/src/index.ts)

<a id="deepseek-aidsh-tool-skill"></a>

## `@deepseek-ai/dsh-tool-skill`

필수: `agents` · `tools` · `skills`
...
출처: [`packages/skill/tool-skill/src/index.ts:61`](../packages/skill/tool-skill/src/index.ts)
...
출처: [`packages/fs/tool-str-replace-editor/src/index.ts:497`](../packages/fs/tool-str-replace-editor/src/index.ts)

<a id="deepseek-aidsh-tool-subagent"></a>

## `@deepseek-ai/dsh-tool-subagent`

필수: `tools` · `subagents` · `systemPrompt`
...
의존 대상: [`AgentOptions`](subsystems/core.md)

출처: [`packages/subagent/tool-subagent/src/index.ts:29`](../packages/subagent/tool-subagent/src/index.ts)

<a id="deepseek-aidsh-tool-subagent-report"></a>

## `@deepseek-ai/dsh-tool-subagent-report`

필수: `subagents` · `tools` · `systemPrompt`
...
의존 대상: [`SubagentReportDelivery`](subsystems/subagent.md)

출처: [`packages/subagent/tool-subagent-report/src/index.ts:27`](../packages/subagent/tool-subagent-report/src/index.ts)

<a id="deepseek-aidsh-tool-terminal"></a>

## `@deepseek-ai/dsh-tool-terminal`

필수: `terminals` · `tools` · `systemPrompt`
...
소스: [`packages/terminal/tool-terminal/src/index.ts:35`](../packages/terminal/tool-terminal/src/index.ts)
...
소스: [`packages/todo/tool-todo/src/index.ts:29`](../packages/todo/tool-todo/src/index.ts)

<a id="deepseek-aidsh-tool-web"></a>

## `@deepseek-ai/dsh-tool-web`

필수: `tools` · `web` · `systemPrompt`
...
소스: [`packages/web/tool-web/src/index.ts:37`](../packages/web/tool-web/src/index.ts)

<a id="deepseek-aidsh-tool-workflow"></a>

## `@deepseek-ai/dsh-tool-workflow`

필수: `tools` · `workflowEngine` · `systemPrompt`
...
소스: [`packages/workflow/tool-workflow/src/index.ts:33`](../packages/workflow/tool-workflow/src/index.ts)
...
소스: [`packages/core/tools/src/index.ts:654`](../packages/core/tools/src/index.ts)
...
소스: [`packages/typert/loader/src/index.ts:47`](../packages/typert/loader/src/index.ts)
...
소스: [`packages/interaction/user-approval/src/index.ts:177`](../packages/interaction/user-approval/src/index.ts)
...
소스: [`packages/web/web/src/index.ts:55`](../packages/web/web/src/index.ts)
...
소스: [`packages/bundle/web-app/src/index.ts:38`](../packages/bundle/web-app/src/index.ts)
...
출처: [`packages/web/web-fetch-http/src/index.ts:34`](../packages/web/web-fetch-http/src/index.ts)
...
출처: [`packages/web/web-search-deepseek/src/index.ts:46`](../packages/web/web-search-deepseek/src/index.ts)
...
출처: [`packages/web/web-search-exa/src/index.ts:38`](../packages/web/web-search-exa/src/index.ts)
...
출처: [`packages/web/web-search-perplexity/src/index.ts:32`](../packages/web/web-search-perplexity/src/index.ts)
...
출처: [`packages/workflow/workflow-worker-thread/src/index.ts:32`](../packages/workflow/workflow-worker-thread/src/index.ts)

## 설정이 없는 로드 가능한 플러그인

이러한 플러그인은 `cordis.yml` 항목에서 로드되며 `config:` 블록이 없습니다. 구성 API도 선언하지 않습니다.

- `@deepseek-ai/dsh-agent` ([`packages/core/agent/src/index.ts`](../packages/core/agent/src/index.ts))
- `@deepseek-ai/dsh-api-gateway` — `typert` 필요 ([`packages/api/gateway/src/index.ts`](../packages/api/gateway/src/index.ts))
- `@deepseek-ai/dsh-api-remotes` ([`packages/api/remotes/src/index.ts`](../packages/api/remotes/src/index.ts))
- `@deepseek-ai/dsh-client-locale` ([`packages/client/locale/src/index.ts`](../packages/client/locale/src/index.ts))
- `@deepseek-ai/dsh-client-modules` — `webServer` · `loader` 필요 ([`packages/client/modules/src/index.ts`](../packages/client/modules/src/index.ts))
- `@deepseek-ai/dsh-client-runtime` ([`packages/client/runtime/src/index.ts`](../packages/client/runtime/src/index.ts))
- `@deepseek-ai/dsh-client-ui-agent-preset` ([`packages/client/ui-agent-preset/src/index.ts`](../packages/client/ui-agent-preset/src/index.ts))
- `@deepseek-ai/dsh-client-ui-commands` ([`packages/client/ui-commands/src/index.ts`](../packages/client/ui-commands/src/index.ts))
- `@deepseek-ai/dsh-client-ui-conversation` ([`packages/client/ui-conversation/src/index.ts`](../packages/client/ui-conversation/src/index.ts))
- `@deepseek-ai/dsh-client-ui-cordis` ([`packages/extensions/ui-cordis/src/index.ts`](../packages/extensions/ui-cordis/src/index.ts))
- `@deepseek-ai/dsh-client-ui-deliverables` — `systemPrompt` 필요 ([`packages/client/ui-deliverables/src/index.ts`](../packages/client/ui-deliverables/src/index.ts))
...
- `@deepseek-ai/dsh-command-compact` — `commands` · `compaction` 필요 ([`packages/compaction/command-compact/src/index.ts`](../packages/compaction/command-compact/src/index.ts))
- `@deepseek-ai/dsh-command-feedback` — `commands` 필요 ([`packages/feedback/command-feedback/src/index.ts`](../packages/feedback/command-feedback/src/index.ts))
- `@deepseek-ai/dsh-command-goal` — `commands` · `goals` 필요 ([`packages/goal/command-goal/src/index.ts`](../packages/goal/command-goal/src/index.ts))
- `@deepseek-ai/dsh-commands` ([`packages/interaction/commands/src/index.ts`](../packages/interaction/commands/src/index.ts))
- `@deepseek-ai/dsh-cordis-client-runner` ([`packages/extensions/cordis-client-runner/src/index.ts`](../packages/extensions/cordis-client-runner/src/index.ts))
- `@deepseek-ai/dsh-fs-e2b` — `e2b` 필요 ([`packages/e2b/fs-e2b/src/index.ts`](../packages/e2b/fs-e2b/src/index.ts))
- `@deepseek-ai/dsh-fs-observation-policy` ([`packages/fs/fs-observation-policy/src/index.ts`](../packages/fs/fs-observation-policy/src/index.ts))
- `@deepseek-ai/dsh-goal-round-driver` — `agents` · `goals` · `sessions` 필요 ([`packages/goal/goal-round-driver/src/index.ts`](../packages/goal/goal-round-driver/src/index.ts))
- `@deepseek-ai/dsh-host-directory-picker-auto` — `webServer` · `loader` 필요 ([`packages/host/directory-picker-auto/src/index.ts`](../packages/host/directory-picker-auto/src/index.ts))
- `@deepseek-ai/dsh-host-directory-picker-native` ([`packages/host/directory-picker-native/src/index.ts`](../packages/host/directory-picker-native/src/index.ts))
- `@deepseek-ai/dsh-host-plugin-inventory` — `loader` 필요 ([`packages/host/plugin-inventory/src/index.ts`](../packages/host/plugin-inventory/src/index.ts))
- `@deepseek-ai/dsh-llm` ([`packages/llm/llm/src/index.ts`](../packages/llm/llm/src/index.ts))
- `@deepseek-ai/dsh-lsp` ([`packages/lsp/lsp/src/index.ts`](../packages/lsp/lsp/src/index.ts))
- `@deepseek-ai/dsh-schedule` — `agents` · `sessions` · `tools` · `sessionPersistence` 필요 ([`packages/schedule/schedule/src/index.ts`](../packages/schedule/schedule/src/index.ts))
- `@deepseek-ai/dsh-session` ([`packages/core/session/src/index.ts`](../packages/core/session/src/index.ts))
- `@deepseek-ai/dsh-session-checkpoint-policy` — `llm` · `sessionPersistence` · `sessions` · `tools` 필요 ([`packages/session/session-checkpoint-policy/src/index.ts`](../packages/session/session-checkpoint-policy/src/index.ts))
- `@deepseek-ai/dsh-session-log-export` — `commands` 필요 ([`packages/session-query/session-log-export/src/index.ts`](../packages/session-query/session-log-export/src/index.ts))
- `@deepseek-ai/dsh-session-projection` ([`packages/session/session-projection/src/index.ts`](../packages/session/session-projection/src/index.ts))
- `@deepseek-ai/dsh-session-stats` — `sessionProjections` 필요 ([`packages/session/session-stats/src/index.ts`](../packages/session/session-stats/src/index.ts))
- `@deepseek-ai/dsh-skill-badge` — `skills` 필요 ([`packages/skill/skill-badge/src/index.ts`](../packages/skill/skill-badge/src/index.ts))
- `@deepseek-ai/dsh-storage` ([`packages/storage/storage/src/index.ts`](../packages/storage/storage/src/index.ts))
- `@deepseek-ai/dsh-subagent` ([`packages/subagent/subagent/src/index.ts`](../packages/subagent/subagent/src/index.ts))
- `@deepseek-ai/dsh-subprocess-local` ([`packages/subprocess/subprocess-local/src/index.ts`](../packages/subprocess/subprocess-local/src/index.ts))
- `@deepseek-ai/dsh-terminal` ([`packages/terminal/terminal/src/index.ts`](../packages/terminal/terminal/src/index.ts))
- `@deepseek-ai/dsh-tool-ask-user` — `tools` · `userQuestions` 필요 ([`packages/interaction/tool-ask-user/src/index.ts`](../packages/interaction/tool-ask-user/src/index.ts))
- `@deepseek-ai/dsh-tool-call-timeout-policy` — `tools` 필요 ([`packages/guard/timeout-policy/src/index.ts`](../packages/guard/timeout-policy/src/index.ts))
- `@deepseek-ai/dsh-tool-cordis` — `tools` · `systemPrompt` · `dynamicCordisRunner` · `cordisInspect` 필요 ([`packages/extensions/tool-cordis/src/index.ts`](../packages/extensions/tool-cordis/src/index.ts))
- `@deepseek-ai/dsh-tool-subagent-control` — `tools` · `subagents` 필요 ([`packages/subagent/tool-subagent-control/src/index.ts`](../packages/subagent/tool-subagent-control/src/index.ts))
- `@deepseek-ai/dsh-user-questions` ([`packages/interaction/user-questions/src/index.ts`](../packages/interaction/user-questions/src/index.ts))
- `@deepseek-ai/dsh-workspace` — `storageDomain` · `sessionPersistence` 필요 ([`packages/workspace/workspace/src/index.ts`](../packages/workspace/workspace/src/index.ts))

## 연결점 패키지(직접 로드할 수 없음)

추상 서비스 클래스 — 배포에서는 대신 구체적인 구현 패키지를 로드합니다([기능 연결점](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)).

- `@deepseek-ai/dsh-attachment` — 추상 `AttachmentStore` ([`packages/attachment/attachment/src/index.ts`](../packages/attachment/attachment/src/index.ts))
- `@deepseek-ai/dsh-code-runtime` — 추상 `CodeRuntime` ([`packages/code-runtime/code-runtime/src/index.ts`](../packages/code-runtime/code-runtime/src/index.ts))
- `@deepseek-ai/dsh-compaction` — 추상 `CompactionEngine` ([`packages/compaction/compaction/src/index.ts`](../packages/compaction/compaction/src/index.ts))
- `@deepseek-ai/dsh-credentials` — 추상 `CredentialProvider` ([`packages/credentials/credentials/src/index.ts`](../packages/credentials/credentials/src/index.ts))
- `@deepseek-ai/dsh-fs` — 추상 `FileSystem` ([`packages/fs/fs/src/index.ts`](../packages/fs/fs/src/index.ts))
- `@deepseek-ai/dsh-host-directory-picker` — 추상 `DirectoryPicker` ([`packages/host/directory-picker/src/index.ts`](../packages/host/directory-picker/src/index.ts))
- `@deepseek-ai/dsh-jobs` — 추상 `JobRegistry` ([`packages/jobs/jobs/src/index.ts`](../packages/jobs/jobs/src/index.ts))
- `@deepseek-ai/dsh-sandbox` — 추상 `SandboxProvider` ([`packages/sandbox/sandbox/src/index.ts`](../packages/sandbox/sandbox/src/index.ts))
- `@deepseek-ai/dsh-session-persistence` — 추상 `SessionPersistence` ([`packages/session/session-persistence/src/index.ts`](../packages/session/session-persistence/src/index.ts))
- `@deepseek-ai/dsh-session-query` — 추상 `SessionQueryEngine` ([`packages/session-query/session-query/src/index.ts`](../packages/session-query/session-query/src/index.ts))
- `@deepseek-ai/dsh-settings` — 추상 `SettingsProvider` ([`packages/settings/settings/src/index.ts`](../packages/settings/settings/src/index.ts))
- `@deepseek-ai/dsh-shell` — 추상 `ShellExecutor` ([`packages/shell/shell/src/index.ts`](../packages/shell/shell/src/index.ts))
- `@deepseek-ai/dsh-spill` — 추상 `SpillStore` ([`packages/spill/spill/src/index.ts`](../packages/spill/spill/src/index.ts))
- `@deepseek-ai/dsh-subprocess` — 추상 `SubprocessRuntime` ([`packages/subprocess/subprocess/src/index.ts`](../packages/subprocess/subprocess/src/index.ts))
- `@deepseek-ai/dsh-workflow` — 추상 `WorkflowEngine` ([`packages/workflow/workflow/src/index.ts`](../packages/workflow/workflow/src/index.ts))

## 라이브러리 패키지(플러그인 항목 없음)

다른 패키지에서 라이브러리로 가져오며, `cordis.yml`로는 이를 로드할 수 없습니다.
