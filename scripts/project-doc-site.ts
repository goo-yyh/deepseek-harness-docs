...
interface UpstreamTreeFile {
  path: string
  type: 'blob' | 'tree' | 'commit'
}

interface UpstreamTreeIndex {
  commit: string
  entries: UpstreamTreeFile[]
}

const upstreamTree = JSON.parse(
  readFileSync(resolve(root, 'config/upstream-tree.json'), 'utf8'),
) as UpstreamTreeIndex
const upstreamEntries = new Map(upstreamTree.entries.map(entry => [entry.path, entry.type]))
...
function repositoryTarget(absPath: string, repoRoot: string): { absPath: string; upstreamKind: 'blob' | 'tree' } | undefined {
  if (existsSync(absPath)) return { absPath, upstreamKind: lstatSync(absPath).isDirectory() ? 'tree' : 'blob' }
  const kind = upstreamEntries.get(repoPath(absPath, repoRoot))
  return kind === 'blob' || kind === 'tree' ? { absPath, upstreamKind: kind } : undefined
}
...
function resolveRepositoryTarget(
  ...
  rawPath: string,
  repoRoot: string,
): { absPath: string; line?: number; upstreamKind: 'blob' | 'tree' } {
  const decoded = decodePath(rawPath)
  let absPath = resolve(dirname(sourceAbs), decoded)
  const direct = repositoryTarget(absPath, repoRoot)
  ...
  if (lineMatch !== null) {
    ...
    const lined = repositoryTarget(absPath, repoRoot)
    if (lined !== undefined) return { ...lined, line: Number.parseInt(lineText, 10) }
  }

  if (extname(decoded) === '') {
    const markdown = resolve(dirname(sourceAbs), `${decoded}.md`)
    const markdownTarget = repositoryTarget(markdown, repoRoot)
    if (markdownTarget !== undefined) return markdownTarget
    const index = resolve(dirname(sourceAbs), decoded, 'index.md')
    const indexTarget = repositoryTarget(index, repoRoot)
    if (indexTarget !== undefined) return indexTarget
  }
...
function githubTarget(
  ...
  return `${REPOSITORY_URL}/${upstreamKind}/${repositoryRef}/${path}${lineSuffix}`
}
...
export function rewriteMarkdown(source: string, options: RewriteMarkdownOptions): string {
  ...
  const rewrite = (node: RewritableNode): void => {
    ...
    if (path === '') return
    const { absPath, line, upstreamKind } = resolveRepositoryTarget(sourceAbs, path, options.repoRoot)
    ...
    const nextUrl = page !== undefined
      ...
      : node.type === 'image' && options.placeImage !== undefined
        ...
        : githubTarget(absPath, upstreamKind, line, suffix, options.repositoryRef, options.repoRoot, node.type === 'image')
...
function referencedImages(): string[] {
  ...
  for (const page of docsPages) {
    ...
    rewriteMarkdown(readFileSync(sourceAbs, 'utf8'), {
      ...
      pages: docsPages,
      repoRoot: root,
      repositoryRef: upstreamTree.commit,
      placeImage: (absPath) => {
...
export function projectDocs(): void {
  ...
  const repositoryRef = process.env.GITHUB_SHA ?? upstreamTree.commit
